...
describe('IHTMLWriter', () => {
  ...
  it('should add a tag properly using addTag()', () => {
    htmlWriter.addTag('floog', CloseTagType.Normal, CanHaveAttributes.CannotHaveAttributes);
    expect(htmlWriter.HTML.toString()).to.equal(`<${stringConstants.cHTML}><floog`);
  });

  it('should close a tag properly', () => {
    htmlWriter.addTag('floog', CloseTagType.Normal, CanHaveAttributes.CannotHaveAttributes).closeTag().closeTag();
  ...
  it('should open <meta> tag properly', () => {
    htmlWriter.openHead().openMeta();
    expect(htmlWriter.HTML.toString()).to.equal(`<${stringConstants.cHTML}><head><meta`);
  });
});
